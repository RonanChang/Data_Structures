...
class Matrix :
    ...
    def scaleBy( self, scalar ):
        ...
        for i in range(self.numRows()):
            for j in range(self.numCols()):
                ...
                self.__setitem__((i,j),value*scalar)
    ...
    def transpose( self ):
        ...
        row = self.numRows()
        col = self.numCols()
        new_array = Matrix(col,row)
        
        for i in range(row):
            for j in range(col):
                value = self.__getitem__((i,j))
                new_array.__setitem__((j,i),value)
    ...
    def __add__( self, rhsMatrix ):
        ...
        row = self.numRows()
        col = self.numCols()
        new_array = Matrix(row,col)
        
        for i in range(row):
            for j in range(col):
                value1 = self.__getitem__((i,j))
                value2 = rhsMatrix.__getitem__((i,j))
                new_array.__setitem__((i,j),value1+value2)
