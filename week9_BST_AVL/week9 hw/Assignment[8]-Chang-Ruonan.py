...
class TreeMap(LinkedBinaryTree, MapBase):
    ...
    def buildTree(self,lst,low,high):
        ...
        mid = (low + high) // 2
        tree = TreeMap()
        tree[lst[mid][0]] = lst[mid][1]
        tree._attach(tree.root(),tree.buildTree(lst,low,mid-1),tree.buildTree(lst,mid+1,high))
        return tree
            
    def to_balanced(self):
        lst = [(p.key(),p.value()) for p in self.inorder()]       
        return self.buildTree(lst,0,len(lst)-1)
        ...
        while p is not None:            
            if self.parent(p) is not None:
                ...
                if p == self.right(self.parent(p)):
                    self.parent(p)._node._balance_factor -= 1                    
                if self.parent(p)._node._balance_factor == 0:
                    return
            p = self.parent(p)
