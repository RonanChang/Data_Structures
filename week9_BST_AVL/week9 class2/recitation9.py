...
class TreeMap(LinkedBinaryTree, MapBase):
    ...
    def min_key(self):
        ...
        # Task 2
        root = self.root()
        while self.left(root) is not None:
            root = self.left(root)
        return root.key()
    ...
    def is_valid(self):
        ...
        def balance(p,name):
            if name.is_leaf(p):
                ...
                if not name.left(p) is None:
                    balance(name.left(p),name)
                    l.append(p.key())
                if not name.right(p) is None:
                    balance(name.right(p),name)
        root = self.root()
        balance(root,self)
